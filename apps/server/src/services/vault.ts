...
export class VaultService {
  ...
  async list(scope: Scope, filter: { clientId?: string; archived?: boolean }): Promise<PasswordView[]> {
    ...
    return visible.map((r) => this.view(r, reuse, links, folders));
  }
  ...
  async get(scope: Scope, id: string): Promise<PasswordView> {
    ...
    return this.view(row, await this.reuseCounts(scope, [row.p]), await this.linkedAssets(scope, [row.p.id]), folders);
  }
