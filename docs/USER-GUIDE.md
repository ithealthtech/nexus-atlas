...
- **Passkeys:** Windows Hello, Touch ID, your phone, or a security key. Add one under **Profile & sign-in → Passkeys** (from the account menu), then use **Sign in with a passkey** without typing a password.
...
- **Forgot your password?** Use **Forgot your password?** on the sign-in page; the link in the email works once, for an hour.
- **Signed in somewhere you shouldn't be?** **Profile & sign-in → Where you're signed in** lists your sessions; sign any of them out.
...
- The eye button (**Show password**) shows a password, and **Copy password** puts it on the clipboard (cleared after 30 seconds), and the six-digit code for accounts with one-time codes updates live. Some clients require a short reason first. Every reveal and copy is recorded.
...
- **Rotation:** set how often a password should change, and it appears in **Expirations** when it's due. Changing it keeps the old one under **Previous passwords**.
